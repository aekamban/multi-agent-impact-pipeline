...
Writes (only to state.reporting):
    ...
    state.reporting.map_export_json    Partner map format dict (gated)
...
Design rules:
    ...
    - map_export_json is gated on jotform_submitted=True to avoid
      publishing incomplete or unconfirmed projects to the organization's public
      map backend.
...
# Real Jotform-style column headers, matching the org's actual export format (exact strings).
...
JF_MERCH_CONSENT   = (
    ...
    "free program merchandise as a thank-you! If you'd like to be considered, "
    "kindly provide your address."
)
JF_MAILING_ADDRESS = "Mailing address, for shipping your free program merchandise:"
JF_CONSENT         = (
    "By submitting this form, you agree that the organization can use this material for "
...
JOTFORM_PRIVACY_NOTE = (
    "This draft was generated automatically. "
...
JOTFORM_BLANK_FIELDS = [
    ...
    "free program merchandise as a thank-you! If you'd like to be considered, "
    "kindly provide your address.",
    "Mailing address, for shipping your free program merchandise:",
    "By submitting this form, you agree that the organization can use this material for "
    "promotion, marketing, and dissemination.",
]
...
# and INTERMEDIATE OUTCOMES that a program teacher would recognise as accurate.
...
def build_logic_model(
    ...
    lab_name     = _safe_str(intake.get("canonical_lab_name")) or "the program's Learning Lab"
    ...
    # Build INPUTS
    inputs: list[str] = [f"{lab_name} curriculum"]
    if student_count:
...
def build_jotform_draft(
    ...
    """
    Build a draft dict keyed by the org's real Jotform column headers.
    ...
    Design principles:
      ...
      - Media / upload fields remain blank — project artifacts are described
        as "to be submitted via the organization's official form", never implied to be
        stored here.
    ...
    if not overview:
        ...
        parts = []
        if lab_name:
            parts.append(f"{lab_name} action project.")
        if thematic:
    ...
    highlights_parts.append(
        ...
        "via the organization's official Jotform — they are not stored in this system."
    )
...
def build_funder_summary_deterministic(
    ...
    """
    lab_name      = _safe_str(intake.get("canonical_lab_name")) or "a program Learning Lab"
    ...
    summary_parts = [
        ...
        f"as part of the program's {lab_name} curriculum."
    ]
    ...
    if equity is True or _is_title1(teacher_context):
        summary_parts.append(
            ...
            "reflecting the program's commitment to climate equity."
        )
...
def build_map_export(
    ...
    """
    Build the partner map export JSON.

    Gate: map_export_json is only generated when jotform_submitted=True.

    Rationale: The public map displays confirmed, complete projects.
    Publishing an incomplete or in-progress record would misrepresent the org's
    ...
    export: dict[str, Any] = {
        ...
        # The org's map backend should resolve lat/lng from city+country.
        "lat":                       None,
        "lng":                       None,
    }
